...
export const getItem = async (req, res) => {
  try {
    const { lname } = req.params;
    const decodedLinkname = decodeURIComponent(lname);

    // Find by short linkname only (exact match recommended)
    const item = await Item.findOne({ linkname: decodedLinkname });
...
export const addItem = async (req, res) => {
  try {
    const fullLink = req.body.whatsappgrouplink;
    ...
    const newItem = new Item({
      name: req.body.name,
      linkname: groupCode, // safe short ID for route
      link: fullLink,      // full group URL for frontend redirect
      category: categoryDoc._id,
      image1: imageUrls[0],
      image2: imageUrls[1],
      image3: imageUrls[2],
